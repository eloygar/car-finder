...
describe('seedKnownIssues', () => {
  it('upserts deterministic records and never deletes unrelated data', async () => {
    ...
    const knownIssueUpsert = vi.fn((args: unknown) => Promise.resolve(args));
    const vehicleModelUpsert = vi.fn((args: unknown) => Promise.resolve(args));
    const transaction = vi.fn((operations: Promise<unknown>[]) => Promise.all(operations));
    const deleteMany = vi.fn();
    const prisma = {
      knownIssue: { findMany, upsert: knownIssueUpsert, deleteMany },
    ...
    expect(knownIssueUpsert).toHaveBeenCalledTimes(12);
    expect(vehicleModelUpsert).toHaveBeenCalledTimes(6);
